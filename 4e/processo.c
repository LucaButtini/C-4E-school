...
// a livello globale, "p" potrebbe essere utilizzata anche in altre funzioni.
// a livello locale dentro ad una funzione, al termine di essa viene deallocata
int main()
{
    // int p;
    ...
    printf("ciao\n");
    // questa istruzione prima della funzione fork viene lo stesso eseguita, dopo vengono eseguite le istruzioni della fork

    p = fork(); // restituisce un intero
    if (p != 0) // siamo nel padre
    {
        printf("\nIo sono il processo padre, p=%d, PID=%d, mio padre ha PID uguale %d \n", p, getpid(), getppid()); // funzione getpid permette di ottenere la visualizzazione del PID. Il padre del padre è il terminale, la shell bash.
    }
    else // figlio
    {
        printf("\nIo sono il processo figlio, p=%d, PID=%d, mio padre ha PID uguale %d\n", p, getpid(), getppid()); // funzione getppid restituisce pid del padre
    }
    return 0;
}