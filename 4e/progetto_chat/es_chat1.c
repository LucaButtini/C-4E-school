...
int main()
{
    ...
    if (p > 0)
    {
        if ((fd = open("fifo1", O_WRONLY)) < 0)
            Error("Errore nell'apertura di fifo1 in writeonly");
        buffer = malloc(STRING_LEN);
        while (1)
        {
            printf(ANSI_COLOR_GREEN "CHAT1: ");
    ...
    else
    {
        if ((fd = open("fifo2", O_RDONLY)) < 0)
            Error("Errore nell'apertura di fifo2 in readonly");
        buffer = malloc(STRING_LEN);
        while (1)
        {
            ...
            printf(ANSI_COLOR_BLUE "CHAT2: %s" ANSI_COLOR_GREEN "CHAT1: ", buffer);
            fflush(stdout);
            if (strcmp(buffer, "HALT\n\0") == 0)
            {
                printf("\nTerminazione programma\n");
